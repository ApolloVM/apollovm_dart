...
- **`PackageProvider`** — the pluggable fetch strategy:
  - **`PackageConfigProvider`** (default, VM-only, zero extra deps): resolves via
    `.dart_tool/package_config.json` (generated by `dart pub get` from the pubspec, populated
    from pub.dev) → exact pub semantics. Filesystem-based, so it is a no-op on the web.
  - **`PubDevProvider`** (opt-in, **web-compatible**): downloads package archives from a pub
    host (`https://pub.dev` by default, or a configurable/private/mirror host), extracts them
    **in memory**, and caches them (`MemoryPackageCache` by default; `FilePackageCache` on the
    VM), honoring pubspec version constraints (best-effort highest-satisfying version — not a
    full solver). Built only on web-safe libraries (`http`, `archive`, `pub_semver`, `yaml`) —
    no `dart:io` — so it runs on the VM **and** in the browser.
...
- Injected via `ApolloVM.moduleLoader`; usable from the CLI with `apollovm run --pub`
  (`--pub-host` selects the network provider, `--pub-cache` sets its disk cache dir).
...
`PubDevProvider` is web-safe, but a **browser** cannot fetch `https://pub.dev` (or the
Google-Cloud-hosted archives) cross-origin — pub.dev sends no permissive CORS headers. Point the
provider at a CORS-enabled host, inject a proxying `http.Client`, or route every request through a
proxy with `rewriteUrl`. A ready-made proxy ships in `tool/pub_cors_proxy.dart`:
...
final provider = PubDevProvider(
  ...
  // cache: MemoryPackageCache() (default on web),
  // pubspecYaml: '<your pubspec.yaml contents>' for version constraints.
);
vm.moduleLoader = DartPackageLoader(vm, provider);
await (vm.moduleLoader as DartPackageLoader).provision();
```
